...
CLIENT_ID = 'scale001'
POLL_INTERVAL = 10  # Интервал опроса сервера (секунды)
...
class Pos:
    ...
    def _connect(self):
        logging.info(f"Попытка подключения к дисплею {self.port} на {self.baudrate}")
        try:
            ...
            logging.info(f"Подключение к дисплею установлено: {self.ser.is_open}")
            return True
        except Exception as e:
            logging.error(f"Ошибка подключения к дисплею: {str(e)}")
            self.ser = None
            self._ready_state = False
            return False
    ...
    def try_reconnect(self):
        ...
        logging.info(f"Попытка переподключения к дисплею #{self._connection_attempts}")
        
        if self._connect():
            return True
        else:
            if self._connection_attempts >= self._max_connection_attempts:
                logging.warning(f"Достигнуто максимальное количество попыток подключения к дисплею ({self._max_connection_attempts})")
    ...
    def initialize_display(self):
        ...
        try:
            self.ser.write(bytes.fromhex('0C')) # очистка дисплея
            self.ser.write(bytes.fromhex('1B7406')) # переключить на русскую кодировку
            self.ser.write(bytes("ВЕС            СУММА", 'cp866')) # печать
            self.ser.write(bytes.fromhex('0B0A')) # С новой строки
            self.ser.write(bytes("ЦЕНА", 'cp866')) # печать
            return True
        except serial.SerialException as e:
            logging.error(f"Ошибка инициализации дисплея: {e}")
            self._ready_state = False
            return False
    ...
    def send(self, text):
        ...
        try:
            self.ser.write(bytes.fromhex('0C')) # очистка дисплея
            self.ser.write(bytes.fromhex('1B7406')) # переключить на русскую кодировку
            self.ser.write(bytes(text, 'cp866')) # печать
            return True
        except serial.SerialException as e:
            logging.error(f"Ошибка отправки на дисплей: {e}")
            self._ready_state = False
            return False

    def scaleFormat(self, bin_val, n, delimiter=0, koeff='0'):
        val = int(bin_val)
        str_val = str(val)

        if val < 10 and delimiter > 0:
            str_val = "0" + str_val

        if val < 100 and delimiter == 3:
            str_val = "0" + str_val
        ...
        if delimiter > 0:
            str_val = str_val[:-delimiter] + '.' + str_val[-delimiter:]
            n = n - 1
        ...
        return bytes(' '*(n-len(str_val)) + str_val, 'cp866')

    def print_status(self, status_data):
        if not status_data or not self.is_ready():
            if not self.is_ready():
                self.try_reconnect()
            return False
        ...
        try:
            ...
            total = status_data.get('sum', 0)
            bits = status_data.get('bits', {})

            weight_str = self.scaleFormat(str(abs(weight)), 8, 3, '1' if weight < 0 else '0')
            price_str = self.scaleFormat(str(price), 8, 2)
            total_str = self.scaleFormat(str(total), 9, 2)

            self.ser.write(bytes.fromhex('1F240601')) # курсор
            self.ser.write(weight_str) # печать веса
            self.ser.write(bytes.fromhex('1F240602')) # курсор
            self.ser.write(price_str) # печать цены
            self.ser.write(total_str) # печать суммы

            if bits.get('tare_mode', False):
                self.ser.write(bytes.fromhex('1F240401')) # курсор
                self.ser.write(bytes("-т", 'cp866'))
            else:
                self.ser.write(bytes.fromhex('1F240401'))
                self.ser.write(bytes("  ", 'cp866'))
                
            return True
        except serial.SerialException as e:
            logging.error(f"Ошибка вывода статуса на дисплей: {e}")
            self._ready_state = False
            return False
    ...
    def close(self):
        if self.ser and self.ser.is_open:
            self.ser.close()
            logging.info(f"Порт дисплея {self.port} закрыт")
        self._ready_state = False
...
class ScaleClient:
    def __init__(self, port=SERIAL_PORT, baudrate=BAUDRATE):
        ...
        self._last_reconnect_attempt = 0
        self._connection_attempts = 0
        self._max_connection_attempts = 5000
        self._current_status = None
        
        # Инициализация дисплея с обработкой возможных ошибок
        try:
            self.display = Pos(DISPLAY_PORT, DISPLAY_BAUDRATE)
            if not self.display.is_ready():
                logging.warning("Дисплей не подключен, продолжение работы без дисплея")
        except Exception as e:
            logging.error(f"Ошибка инициализации дисплея: {e}")
            self.display = None
    ...
    def _wait_ready(self, timeout=10.0) -> bool:
        ...
        while time.time() - start < timeout:
            ...
            logging.debug(f"получено от весов {byte.hex()}")
            if not byte:
                continue
            if byte == b'\xEE':
    ...
    def _send_command(self, cmd: bytes, data: bytes = b'', expected_len: int = None) -> bytes:
        ...
        try:
            self.ser.reset_input_buffer()
            logging.debug(f"Отправка команды {cmd}, данные: {data.hex()}")
    ...
    def get_current_status(self) -> dict:
        ...
        # Попытка вывода на дисплей (если доступен)
        if hasattr(self, 'display') and self.display:
            if not self.display.print_status(status_data):
                logging.warning("Не удалось отправить данные на дисплей")
    ...
    def disconnect(self):
        if self.ser and self.ser.is_open:
            self.ser.close()
            logging.info(f"Порт весов {self.port} закрыт")
        self._ready_state = False
...
def main():
    ...
    try:
        ...
        scale_client.display.close()  # Закрытие соединения с дисплеем
